...
const stickerImgTag = async(img, pacname, auhorname, fileame) => {
	return new Promise((resolve, reject) => {
		image = getRandom('_.png')
		webp = getRandom('_.webp')
		exec(`ffmpeg -i ${img} -vf scale=512:512 ${webp}`, async(err) => {
			fs.unlinkSync(img)
...
const stickerVidTag = async(media, pacname, authorname, filename) => {
	webp = getRandom('_.webp')
	webp2 = getRandom('.webp')
	exec(`ffmpeg -i ${media} -y -vcodec libwebp -fs 0.99M -filter_complex "[0:v] scale=512:512,fps=10,pad=512:512:-1:-1:color=white@0.0,split[a][b];[a]palettegen=reserve_transparent=on:transparency_color=ffffff[p];[b][p]paletteuse" -f webp ${webp}`, async function(err, res){
		if(err) return reject('Falha')
		await addExif(pacname, authorname, filename)
		exec(`webpmux -set exif ${filename}.exif ${webp} -o 12${webp2}`, async (err) => {
			if (err) console.log(err);
			resolve({
				status: true,
				result: await fs.readFileSync(webp), 
			})
